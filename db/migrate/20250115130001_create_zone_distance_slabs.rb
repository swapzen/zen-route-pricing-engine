...
class CreateZoneDistanceSlabs < ActiveRecord::Migration[8.0]
  def up
    unless table_exists?(:zone_distance_slabs)
    ...
    # For fast lookups
    add_index :zone_distance_slabs, [:zone_id, :vehicle_type, :active], if_not_exists: true
    add_index :zone_distance_slabs, [:city_code, :active], if_not_exists: true
  end

  def down
    drop_table :zone_distance_slabs, if_exists: true
  end
end
