...
# =====================================================================
# Tests all routes, time bands, and vehicle types against competitor benchmarks
# 
# Usage:
...
# Variance thresholds
MIN_VARIANCE = -3.0  # Can be up to 3% cheaper than benchmark
MAX_VARIANCE = 16.0  # Can be up to 16% more expensive than benchmark (accommodates unit economics guardrail)

# =====================================================================
# Test Scenarios (10 Routes × 3 Time Bands × 7 Vehicles = 210 Tests)
# =====================================================================
# Competitor benchmark prices adjusted for current Google Maps distances (April 2026).
...
def run_tests
  ...
  TEST_SCENARIOS.each_with_index do |scenario, route_idx|
    ...
    TIMES.each do |time_name, time|
      ...
      VEHICLES.each do |vehicle_type|
        ...
        # Use calibration mode to disable dynamic multipliers for accurate benchmark comparison
        ...
        benchmark_price = scenario[:benchmark_prices][time_name][vehicle_type.to_sym]
        
        next unless benchmark_price

        total_count += 1
        diff = our_price - benchmark_price
        diff_pct = ((diff / benchmark_price) * 100).round(1)
        ...
        if SHOW_DETAILS || !passed
          puts "    #{vehicle_type.ljust(18)} | Benchmark: ₹#{benchmark_price.to_s.rjust(5)} | SwapZen: ₹#{our_price.to_i.to_s.rjust(5)} | #{diff_pct >= 0 ? '+' : ''}#{diff_pct.to_s.rjust(6)}% #{status}"
        end
      end
    end
  ...
  if fail_count > 0
    ...
    failures.each do |f|
      puts "  #{f[:route]}"
      puts "    #{f[:time].to_s.capitalize} | #{f[:vehicle].ljust(18)} | Benchmark: ₹#{f[:benchmark]} | SwapZen: ₹#{f[:swapzen]} | #{f[:variance] >= 0 ? '+' : ''}#{f[:variance]}%"
    end
  end
