...
class CreatePricingZoneMultipliers < ActiveRecord::Migration[8.0]
  def up
    unless table_exists?(:pricing_zone_multipliers)
    ...
    add_index :pricing_zone_multipliers, [:city_code, :zone_code], unique: true, if_not_exists: true
  ...
  def down
    drop_table :pricing_zone_multipliers, if_exists: true
  end
end
