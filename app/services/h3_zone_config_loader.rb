...
class H3ZoneConfigLoader
  ...
  def sync_zone_pricing!(zone, pricing_hash, force: false)
    ...
    VEHICLE_TYPES.each do |vehicle_type|
      ...
      morning_rates = pricing_hash.dig('morning', vehicle_type)
      next unless morning_rates
  ...
  def seed_city_configs!(vehicle_defaults)
    ...
    VEHICLE_TYPES.each do |vehicle_type|
      ...
      morning_rates = global_time_rates.dig('morning', vehicle_type) || {}
      vehicle_info = vehicles_config[vehicle_type] || {}

      pc.assign_attributes(
        base_fare_paise: vehicle_info['base_fare_paise'] || morning_rates['base'] || 5000,
        per_km_rate_paise: morning_rates['rate'] || 1000,
        per_min_rate_paise: morning_rates['min_rate'] || 0,
        min_fare_paise: vehicle_info['base_fare_paise'] || morning_rates['base'] || 5000,
